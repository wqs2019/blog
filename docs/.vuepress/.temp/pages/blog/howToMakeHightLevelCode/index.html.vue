<template><div><h1 id="我们应该如何编写高质量的前端代码" tabindex="-1"><a class="header-anchor" href="#我们应该如何编写高质量的前端代码" aria-hidden="true">#</a> 我们应该如何编写高质量的前端代码</h1>
<blockquote>
...
<h2 id="_02-代码维护难点" tabindex="-1"><a class="header-anchor" href="#_02-代码维护难点" aria-hidden="true">#</a> 02 代码维护难点</h2>
<p>为什么说前端的代码难以维护呢？其实主要是出于以下的三点：</p>
<h5 id="浏览器层面" tabindex="-1"><a class="header-anchor" href="#浏览器层面" aria-hidden="true">#</a> 浏览器层面</h5>
<p>浏览器是我们前端人经常打交道的工具，其实是浏览器孕育了前端这个大家庭，也是我们生存的根本。假如用户不需要看东西，不需要进行交互，或者我们就不需要存在了。不知道大家是否了解过全息投影？等到那项技术普及的时候，我们又是做着什么样的工作呢？</p>
...
<h5 id="技术层面" tabindex="-1"><a class="header-anchor" href="#技术层面" aria-hidden="true">#</a> 技术层面</h5>
<p>由于每一家的公司使用的技术都不太一样，事实上很多公司都会有自己内部的框架，我们要使用他们内部的框架与现有的技术框架结合在一起，那也是一件很头疼的事情。即使你是一位刚跳槽进来的员工，也是要学习他们内部的技术框架你才可以进行项目的开发。</p>
...
<h5 id="团队合作" tabindex="-1"><a class="header-anchor" href="#团队合作" aria-hidden="true">#</a> 团队合作</h5>
<p>团队合作才是真正的难题，不像我们自己编写的项目，想怎么写都行，不怕有看不懂的时候。当我们提交代码的时候也是不用经过其他成员的同意，直接push到master分支。这些都是我们个人开发的时候做法，到了公司层面或者项目的层面，那你就直接收拾东西吧。</p>
...
<h2 id="_03-高质量结构代码" tabindex="-1"><a class="header-anchor" href="#_03-高质量结构代码" aria-hidden="true">#</a> 03 高质量结构代码</h2>
<h5 id="语义化" tabindex="-1"><a class="header-anchor" href="#语义化" aria-hidden="true">#</a> 语义化</h5>
<p>HTML5出来之后，添加了很多新的标签和属性，而语义化这个概念就出现在前端人的眼中。之前我们编写结构代码的时候一般会选择使用div与类名/ID名的方式来命名一个模块，所谓DIV+CSS开发模式就是这样由来。那么这种方式可否可行呢？当然是可行的，而且别人用的很舒服，就是cv操作多了点。</p>
...
<h5 id="模块化" tabindex="-1"><a class="header-anchor" href="#模块化" aria-hidden="true">#</a> 模块化</h5>
<p>模块化其实就是跟在语义化之后的，如果你的语义化做的比较好，那么相应的模块化就比较好了。我认为模块化的重点应该关注你的标签选择是否合理，比如文字就使用p/span标签，标题应该使用H1-6标签等。不能所有的文字都是用div这个万能的标签元素，能用p的就不要去用div，因为p本身是针对文字的，有一定的基础样式。</p>
<p>小结：</p>
...
<h2 id="_04-高质量样式代码" tabindex="-1"><a class="header-anchor" href="#_04-高质量样式代码" aria-hidden="true">#</a> 04 高质量样式代码</h2>
<h5 id="盒子模型" tabindex="-1"><a class="header-anchor" href="#盒子模型" aria-hidden="true">#</a> 盒子模型</h5>
<p>一般面试或多或少都会问一下你关于css盒子模型的理解，如果没有准备的小伙伴可能会把内容搞混了。这里再重复一遍：</p>
<ul>
<li>IE：元素的宽度由width+border+padding组成</li>
<li>标准：元素的宽度就是width，本身包括了padding+border</li>
</ul>
<h5 id="样式组织" tabindex="-1"><a class="header-anchor" href="#样式组织" aria-hidden="true">#</a> 样式组织</h5>
<p>关于我们页面的样式如何编写，也是我们要考虑的一个问题，我们关于样式的编写没有对错只有好坏。页面写出来，能够满足自己的期望那就是对的样式，是否合理又是另一方面的事情了。</p>
...
<h5 id="选择器使用" tabindex="-1"><a class="header-anchor" href="#选择器使用" aria-hidden="true">#</a> 选择器使用</h5>
<p>选择器可对某一个节点进行样式编写，有同学可能会说，我能把我的节点样式应用成功就好，还需要关心它怎么使用吗？其实要的，我们先从css选择器的匹配方案说起，选择器是从右向左进行匹配的，.class ul li a p这样的选择器，会先从全局匹配p标签然后再匹配a标签，依次类推。</p>
...
<h5 id="编码风格" tabindex="-1"><a class="header-anchor" href="#编码风格" aria-hidden="true">#</a> 编码风格</h5>
<p>样式风格：关于css的编码风格也是因人而异，一般来说我们应该使用多行的写法，因为这样可读性就更高。假如样式代码写在一行，难免造成阅读困难的情况。后期我们进行项目发布的时候可以进行对代码的压缩。</p>
<p>id/class：id选择器一般使用在全局唯一的元素节点上面，如果确定元素节点唯一的情况下我们就可以使用，但是假如元素节点不唯一，那么就推荐使用class。</p>
<h2 id="_05-高质量的行为代码" tabindex="-1"><a class="header-anchor" href="#_05-高质量的行为代码" aria-hidden="true">#</a> 05 高质量的行为代码</h2>
<h5 id="良好习惯" tabindex="-1"><a class="header-anchor" href="#良好习惯" aria-hidden="true">#</a> 良好习惯</h5>
<p>因为项目涉及到多人开发，每个人自己使用的变量应该自己来维护，这样可以有效避免代码冲突，覆盖正常的代码等。所以我们应该禁止直接在全局的作用域下进行代码的编写，这样很容易对别人的项目模块造成影响。那么我们的避免的方法有哪些呢？</p>
<ul>
<li>团队合作避免冲突</li>
</ul>
...
<h5 id="js分层" tabindex="-1"><a class="header-anchor" href="#js分层" aria-hidden="true">#</a> JS分层</h5>
<p>其实这里的分层和css的分层道理是一样的，也可以参考base层、common层、以及view层的形式。其中base层可以封装不同浏览器的差异，提供统一的接口，完成一些兼容的工作。common层提供可复用的组件，功能是给view层提供组件。common层和base层都可以为view层提供组件，不同的是common层可以提供更庞大的组件。比如一些拖拽功能的实现等。关于view层就是直接对前两层的调用，这里是关于页面逻辑的实现部分，如进行接口的请求等与具体功能需求相关的操作。</p>
<h5 id="实用技巧" tabindex="-1"><a class="header-anchor" href="#实用技巧" aria-hidden="true">#</a> 实用技巧</h5>
<ul>
<li>弹性</li>
</ul>
